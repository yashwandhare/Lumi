...
class ModelDownloader @Inject constructor(
            ...
            when (val code = connection.responseCode) {
                ...
                HttpURLConnection.HTTP_NOT_FOUND, HttpURLConnection.HTTP_GONE ->
                    return DownloadOutcome.Permanent(
                        "Lumi could not find the model file on the server."
                    )
                HttpURLConnection.HTTP_UNAUTHORIZED, HttpURLConnection.HTTP_FORBIDDEN ->
                    ...
                    return DownloadOutcome.Permanent(
                        "The model download now requires an account, which Lumi will not ask you for."
                    )
    ...
    private companion object {
        ...
        /**
         * Slack required on top of the weights before a download is allowed to start.
         *
         * Sized for what the runtime writes *after* the download, not for filesystem comfort. On first
         * load LiteRT-LM compiles kernels next to the model — `xnnpack_cache_*`, an MTP drafter cache,
         * and a program cache — measured at roughly 850MB on the test device. At the previous 256MB the
         * download was allowed to start on a phone that then had no room to load what it fetched, which
         * is the worst version of this failure: 2.6GB of data spent to arrive at an unusable app.
         */
        const val HEADROOM_BYTES = 1_100L shl 20
    }
}
