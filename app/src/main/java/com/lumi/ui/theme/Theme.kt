 ...
 * square full-bleed panel — live in [LumiShape] because Material has no slot for a role.
 */
val CohesiveShapes = Shapes(
    extraSmall = LumiShape.default,
    small = LumiShape.default,
    medium = LumiShape.default,
    large = LumiShape.default,
    extraLarge = LumiShape.default,
)

/**
 * The single theme wrapper. Everything renders inside it.
 *
 * Material 3 components pick up Lumi's colours, type, and shapes from here, so a stock `Button`
...
fun LumiTheme(
    ...
    CompositionLocalProvider(
        LocalLumiSpacing provides LumiSpacing(),
        LocalReducedMotion provides reducedMotion,
        LocalMotionEnabled provides (!reducedMotion && !batterySaver && onScreen),
    ) {
        MaterialTheme(
            colorScheme = if (darkTheme) LumiDarkColorScheme else LumiLightColorScheme,
 ...
 */
val MaterialTheme.spacing: LumiSpacing
    @Composable @ReadOnlyComposable get() = LocalLumiSpacing.current
